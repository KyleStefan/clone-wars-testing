...
function update(seconds) {
  if (state === 'playing') {
    ...
    if (CONFIG.fix === 'custom') {
      nextLaserIn -= seconds;
      if (nextLaserIn <= 0) { addLaser(); nextLaserIn = 2.8; }
      for (const laser of lasers) laser.x -= laser.speed * seconds;
      lasers = lasers.filter((laser) => laser.x + laser.width > 0);
      const half = CONFIG.birdSize / 2;
      if (lasers.some((laser) => birdX + half >= laser.x && birdX - half <= laser.x + laser.width && y + half >= laser.y && y - half <= laser.y + laser.height)) crash();
    }
    const half = CONFIG.birdSize / 2;
    if (y + half >= CONFIG.canvasHeight - CONFIG.groundHeight || y - half <= 0) crash();
    for (const pipe of pipes) {
      if (birdX + half >= pipe.x && birdX - half <= pipe.x + CONFIG.pipeWidth && (y - half <= pipe.gapTop || y + half >= pipe.gapBottom)) crash();
    }
...
function draw() {
  ...
  if (CONFIG.fix === 'custom') {
    ...
    ctx.lineWidth = 2;
    for (const laser of lasers) { ctx.fillRect(laser.x, laser.y, laser.width, laser.height); ctx.strokeRect(laser.x, laser.y, laser.width, laser.height); }
    ctx.restore();
  }
  ...
  if (state === 'playing') {
    ...
    ctx.strokeText(String(score), CONFIG.canvasWidth / 2, 52);
    ctx.fillText(String(score), CONFIG.canvasWidth / 2, 52);
    ctx.restore();
  }
}

function frame(now) {
  const seconds = Math.min(0.05, lastFrame ? (now - lastFrame) / 1000 : 0);
  lastFrame = now;
  update(seconds);
  draw();
